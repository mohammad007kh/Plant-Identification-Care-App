# Plant Identification App Development Guidelines

Auto-generated from all feature plans. Last updated: 2026-07-19

## Active Technologies

- TypeScript 5.x on Node.js 22 LTS + NestJS (API); Next.js App Router + React 18 (web); Drizzle ORM; Redis + BullMQ (queue/cache/scheduling); MUI + Emotion (RTL UI); TanStack Query; Zustand; React Hook Form + Zod; LangChain + LangGraph + OpenAI SDK (AI orchestration behind `PlantAIProvider`); nodemailer (SMTP `MailPort`); Zod (shared `contracts` package) (001-plant-id-care-app)
...
- 001-plant-id-care-app: Added TypeScript 5.x on Node.js 22 LTS + NestJS (API); Next.js App Router + React 18 (web); Drizzle ORM; Redis + BullMQ (queue/cache/scheduling); MUI + Emotion (RTL UI); TanStack Query; Zustand; React Hook Form + Zod; LangChain + LangGraph + OpenAI SDK (AI orchestration behind `PlantAIProvider`); nodemailer (SMTP `MailPort`); Zod (shared `contracts` package)
